...
func RunServer() {
	...
	// 创建 server
	server := newServer(serverOut, rollingWindow)
...
func newServer(serverOut chan struct{}, roll *RollingWindow) *http.Server {
	mux := newServeMux(serverOut, roll)
	return &http.Server{
		Addr:    "127.0.0.1:8088",
		Handler: mux,
	}
...
// NewServeMux 创建路由
func newServeMux(serverOut chan struct{}, roll *RollingWindow) *http.ServeMux {
	mux := http.NewServeMux()
	mux.HandleFunc("/healthz", handleFillter(healthzHandler, roll))
	mux.HandleFunc("/timeout", handleFillter(timeOutHandler, roll))
...
// 增加拦截器
func handleFillter(h http.HandlerFunc, roll *RollingWindow) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		roll.ShowAllBucket()
		glog.V(3).Infoln(fmt.Sprintf("http roll RecordReqResult start..."))
		roll.RecordReqResult(true)
		h(w, r)
	}
}
